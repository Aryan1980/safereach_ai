...
export default function SafePlacesPage() {
  ...
  const [mapCenter, setMapCenter] = useState<Coordinates>({ lat: 28.6139, lng: 77.2090 });
  ...
  const [permissionState, setPermissionState] = useState<'prompt' | 'granted' | 'denied' | 'error' | 'idle'>('idle');
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === 1) {
          ...
          setPermissionState('error');
          setErrorMessage('Unable to obtain high-accuracy GPS position. You can use the search bar above to locate safe havens.');
        }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 space-y-8 text-zinc-100">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/[0.08] pb-8">
        ...
        <div className="glass-panel px-4 py-2.5 rounded-xl flex items-center space-x-3 border border-white/[0.08] font-mono text-xs">
          ...
          <span className="text-white font-bold max-w-[200px] truncate">
            {currentAddress || `${mapCenter.lat.toFixed(4)}°, ${mapCenter.lng.toFixed(4)}°`}
          </span>
        </div>
      </div>
      ...
      {permissionState === 'denied' && (
        <div className="glass-panel rounded-2xl p-5 border border-amber-500/30 text-xs space-y-2">
          <div className="flex items-center space-x-2 font-mono font-bold text-amber-400 uppercase tracking-wider">
            <AlertCircle className="w-4 h-4 shrink-0" />
            <span>Browser Geolocation Restricted</span>
          </div>
          <p className="text-zinc-300 font-light leading-relaxed">
            To automatically detect safe havens around your exact location, browser permission is needed. 
            You can search any address, neighborhood, or landmark using the search console below.
          </p>
          <div className="pt-1">
            <button
              onClick={requestCurrentLocation}
              className="text-white hover:underline font-mono text-[11px] uppercase tracking-wider flex items-center space-x-1"
            >
